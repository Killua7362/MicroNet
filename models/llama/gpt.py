...
from micro.tensor import Tensor,split
...
def regress(inputs,n_tokens_gen,n_head,params):
    ...
    gpt = GPT(Tensor(params['wte'],requires_grad=False),Tensor(params['wpe'],requires_grad=False),n_blocks=len(params['blocks']),n_head=n_head)
    load_params(gpt,params)
    for _ in tqdm(range(n_tokens_gen),'generating'):
        logits = gpt(Tensor(inputs,requires_grad=False))
        logits = logits.data
        print(logits)
